<template>
  <div class="flex flex-col h-full">
    ...
    <div class="px-4 py-3 border-b border-border flex items-center justify-between">
      <h2 class="text-lg font-semibold text-foreground">Dev Logs</h2>
      <ExportDropdown
        :dev-logs="devLogs"
    ...
    <div class="flex-1 overflow-y-auto space-y-3 p-4">
      <!-- Empty state -->
      <div v-if="filteredLogs.length === 0" class="flex flex-col items-center justify-center py-16 text-muted-foreground">
      ...
      :key="log.id"
      class="hover:shadow-md transition-shadow duration-200 overflow-hidden"
    >
      <!-- Collapsed card view -->
        ...
        <div class="flex items-start justify-between gap-3">
          ...
          <div class="flex-1 min-w-0 cursor-pointer" @click="toggleExpand(log.id!)">
            ...
            <div class="flex items-center gap-2 mb-2">
              <span class="text-base font-semibold text-foreground">{{ log.project_name }}</span>
            ...
            <div class="flex items-start gap-2 mb-2">
              ...
              <span
                ...
                class="text-xs px-1.5 py-0.5 rounded bg-muted/50 text-muted-foreground font-medium uppercase tracking-wide shrink-0"
          ...
          <div class="flex items-center gap-2">
            <button
              ...
              class="px-3 py-1.5 text-xs font-medium bg-primary hover:bg-primary/90 text-white rounded transition-colors"
              title="View session replay"
            >
              ▶ Replay
            </button>
            <span class="text-lg text-muted-foreground cursor-pointer" @click="toggleExpand(log.id!)">
      ...
      <div v-if="expandedLogs.has(log.id!)" class="border-t border-border px-4 py-3 space-y-4">
        <!-- Tool Breakdown -->
        ...
        <div v-if="parsedFiles(log).length > 0">
          ...
          <div class="space-y-1 max-h-48 overflow-y-auto">
            <div
              ...
              :key="index"
              class="text-xs font-mono bg-muted/50 px-2 py-1 rounded text-muted-foreground"
            >
              {{ file }}
            </div>
          </div>
