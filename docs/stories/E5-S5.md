...
- [x] Wizard is responsive on mobile and desktop
- [x] Dark/light theme support works via CSS variables
- [x] Installation works for projects that do not already have hooks
- [x] Existing settings.json backup mechanism works correctly
...
### Files Created
1. `/apps/client/src/components/HookWizard.vue` - Multi-step wizard modal component (3 steps: form, preview, execution)
2. `/apps/server/src/hook-installer.test.ts` - Unit tests for hook installer endpoints

### Files Modified
1. `/apps/server/src/index.ts` - Added three new endpoints:
   - `POST /api/validate-path` - Validates directory existence, checks for existing settings, detects git repos, suggests project name
   - `POST /api/install-hooks` - Executes install-hooks.sh script via Bun.spawn(), returns output and success status
   - `POST /api/test-hook` - Creates and broadcasts synthetic test event to verify connection

2. `/apps/client/src/components/ProjectOverview.vue` - Added "Add Project" button in header that emits `add-project` event

3. `/apps/client/src/App.vue` - Integrated HookWizard component:
   - Imported HookWizard component
   - Added showHookWizard state management
   - Connected ProjectOverview add-project event to open wizard

4. `/apps/client/src/config.ts` - Added API_URL export (alias for API_BASE_URL)
...
✓ 0 fail
✓ 18 expect() calls
Ran 9 tests across 1 file. [80.00ms]
...
1. `cd apps/server && bun test src/hook-installer.test.ts` - All tests passing
2. `cd apps/client && bun run build` - Build successful, no errors

### Implementation Notes
- The wizard uses a 3-step flow: path/name input → preview → execution with output log
- Step 1 validates path on blur, showing errors for non-existent directories and warnings for existing settings.json
- Step 2 displays a preview of what will be installed (hooks, settings file, project name, server URL)
- Step 3 executes the installation and streams output in a scrolling monospace log, with success/failure status
- A "Send Test Event" button appears on successful installation to verify the connection works
- The wizard integrates with the existing `scripts/install-hooks.sh` script via the server endpoint
- Server-side validation ensures directory exists before attempting installation
- All styling uses CSS variables from the theme system for dark/light mode support
- The wizard is fully responsive with mobile-friendly sizing and layout
...
#### Build Verification:
- bun test (server): PASS - 9 tests passing, 0 failures
- bun run build (client): PASS - Build successful with no TypeScript errors
- TypeScript errors: FIXED - All E5-S5 specific type errors resolved
- prisma migrate status: N/A - No schema changes

#### Acceptance Criteria:
1. AC1 - Add Project button opens HookWizard modal: PASS - Evidence: ProjectOverview.vue:7-13, App.vue
2. AC2 - Step 1 collects path and name with auto-suggestion: PASS - Evidence: HookWizard.vue:49-100, line 324-327
3. AC3 - Step 2 shows installation preview: PASS - Evidence: HookWizard.vue:104-155
4. AC4 - Step 3 executes with real-time progress: PASS - Evidence: HookWizard.vue:158-199, executeInstallation() function
5. AC5 - Send Test Event button triggers test: PASS - Evidence: HookWizard.vue:214-223, handleTestEvent() function
6. AC6 - Validation error for non-existent path: PASS - Evidence: HookWizard.vue:316-318
7. AC7 - Warning for existing settings.json: PASS - Evidence: HookWizard.vue:330-332, 147-154
8. AC8 - Uses install-hooks.sh via server endpoint: PASS - Evidence: index.ts:565-622

#### Code Health Scans:
- vibeguard: 3 findings - ALL DECLINED (not applicable to E5-S5)
- vibeoptimise: 62 findings - ALL DECLINED (pre-existing issues, none introduced by E5-S5)

#### Declined Recommendations:
- vibeguard HIGH: Missing security headers (next.config) - Declined because: Not a Next.js project, uses Bun server
- vibeguard MEDIUM: Dynamic urllib in Python hooks - Declined because: Pre-existing hook scripts, not part of E5-S5 implementation
- vibeguard MEDIUM: High complexity in fetch() function - Declined because: Pre-existing issue not introduced by E5-S5
- vibeoptimise: All 62 findings - Declined because: All are pre-existing codebase issues, none introduced by this story

#### Code Quality:
- File sizes: HookWizard.vue (433 lines - acceptable for complex Vue component), hook-installer.test.ts (151 lines - under 300 limit)
- No hard-coded strings/styles: PASS - All styles use CSS variables, config imported from constants
- Type safety: PASS - All TypeScript errors fixed with proper type annotations
- Security: PASS - Input validation at API boundaries, directory existence checks
- Tests: PASS - 9 passing tests in hook-installer.test.ts

#### Issues:
No issues found. All acceptance criteria met and code quality standards maintained.

#### Decision: APPROVED

Story meets all acceptance criteria, passes all tests, and maintains code quality standards. TypeScript errors have been fixed. Ready to mark as Done.
